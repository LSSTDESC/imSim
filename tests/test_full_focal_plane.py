...
from imsim.full_focal_plane import gather_photons_by_bounds, OffDetectorPhotons
from imsim.utils import focal_to_pixel

DATA_DIR = Path(__file__).parent / 'data'
OUTPUT_DIR = Path(__file__).parent / 'output'

def test_gather_photons_by_bounds():
    ...
    gathered_photons = gather_photons_by_bounds(bounds, photons, inside=False)
...
def process_off_detector_photons_output(pa_list):
    ...
    # gather_photons_by_bounds across an image's photon pools), create a
...
def test_load_photons_from_single_file():
    ...
    # For R22_S12, all of the photons off-detector, so the x, y, and flux arrays are all empty.
    expected_photons = galsim.PhotonArray.fromArrays(x=np.array([]), y=np.array([]), flux=np.array([]))
...
def test_load_photons_from_multiple_files():
    ...
    # following in the R22_S11 detector coordinates, but only the photons landing
    # on theR22_S11 detector should be retained.
