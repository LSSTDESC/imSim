...
def make_flats(obs_md, photoz_params=None, counts_per_iter=4e3, counts_total=80e3, treering_amplitude = 0.26, nflats=1, nborder=2, treering_period = 47., treering_center = galsim.PositionD(0,0), seed = 31415):
    
    if photoz_params==None:
        photoz_params = PhotometricParameters
    ...
    detectors = [GSInterface.make_galsim_detector(cameraWrapper, det.getName(), photoz_params, obs_md) for det in list(camera) if str(det.getType())=='DetectorType.SCIENCE']
