...
class LSST_PhotonPoolingImageBuilder(LSST_ImageBuilderBase):
    ...
    def buildImage(self, config, base, image_num, _obj_num, logger):
        ...
        offset = self.calc_offset(full_image.bounds)
        ...
        for batch_num, batch in enumerate(phot_batches, start=current_photon_batch_num):
            ...
            base['index_key'] = 'image_num'
            stamps, current_vars = self.build_stamps(base, logger, batch)
            self.offset_photon_arrays(stamps, offset)
    ...
    def calc_offset(bounds):
        ...
        Returns:
            adjustment: a 0 or 0.5 pixel offset to be made to photon positions
        """
        ...
        if shape[0] % 2 == 0:
            ...
            dy = 0.0
        offset = galsim._PositionD(dx,dy)
        return offset

    @staticmethod
    def offset_photon_arrays(stamps, offset):
        ...
        Parameters:
            ...
            offset: a PositionD by which to offset each object
        """
        for stamp in stamps:
